  ...
  if (selectedResult) {
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        ...
        <ResultCard
          score={selectedResult.result.score}
          totalQuestions={selectedResult.assessment.questions.length}
          onRestart={() => {
            onStartAssessment(selectedResult.assessment.id);
            setSelectedResult(null);
          }}
          questions={selectedResult.assessment.questions}
          userAnswers={selectedResult.result.userAnswers || []}
          evaluationResults={selectedResult.result.evaluationResults || []}
        />
      </div>
    );
  }
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                {completedResults.map(result => {
                  ...
                  return (
                    <div 
                      key={result.id} 
                      className="border-4 bg-gray-50 border-gray-500 rounded-lg p-4 hover:border-blue-500 transition-colors cursor-pointer"
                      onClick={() => assessment && setSelectedResult({ result, assessment })}
                    >
